...
- Googleの住所取得は呼び出し頻度を抑制する。自前のNestJS＋PostGISのスポット検索結果は、必要に応じてNext.js等でキャッシュする。キャッシュの配置・方式・有効期間と、住所取得の間隔は未決定。
...
- Next.jsとの接続方法、住所取得の呼び出し経路、頻度・キャッシュ・失敗時表示、キー設定と起動手順を確認する。
